...
class Staff(User):
    ...
    def menu(self):
        print(f"""      Staff Menu
            ...
            """)
        while True:
            c = input("\nSelect Option (1-9|q): ")              # Action functions for each respective selected option on the staff menu
    ...
    def returnBook(self):
        ...
        if borrowRecord and borrowRecord[0] == False:        # this statement ONLY runs when there is a borrowed record and when the isReturned value is false. this signifies that the book has not yet been returned
            ...
            if differenceBetweenDates > 15.0:
                fineIssued = 20
                newTotalFine = result2[2] + fineIssued
                c.execute("""
    ...
    def viewFine(self):
        isbn = input('Please enter isbn: ')
        bookId = LibraryDB.getBookIdByIsbn(isbn)
        myFine = LibraryDB.getUserFineValue(self.id, bookId)
        if myFine == None:
            myFine = 0
    ...
    def finePayment(self):
        ...
        if paymentValue == myFine:            
            ...
            print("Please enter correct fine amount to pay, restart process...")
